...
- Go CLI 是 Python 与 Node CLI 外的第三实现。Python CLI 保持零行为改动；Node CLI 的重写例外见下节。Go CLI 新增命令或选项必须先进入 parity golden。
...
- 大号终端 `LOGO` 展示用户指定的 `MOMGO CLI` 字样，保持品牌面板对齐。终端品牌采用青春明亮配色：亮粉主色、浅青强调色，成功、错误、警告和弱化提示分别使用薄荷绿、珊瑚红、柠檬黄和蓝灰色；色值统一由品牌模块维护。
