...
func (s *Server) Routes() http.Handler {
	...
	r.Get("/webhooks", s.handler(s.listWebhooks, authVerify))
	r.Post("/webhooks", s.handler(s.createWebhook, authAdmin))
	r.Put("/webhooks/{webhook_id}", s.handler(s.updateWebhook, authAdmin))
	r.Delete("/webhooks/{webhook_id}", s.handler(s.deleteWebhook, authAdmin))
