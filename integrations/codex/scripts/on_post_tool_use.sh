#!/usr/bin/env bash
# Hook: PostToolUse — track MemGo MCP tool usage for session stats
#
# Fires after any tool call. We only care about MemGo MCP tools:
#   mcp__memgo__add_memory     → record an add
#   mcp__memgo__search_memories → record a search
#
