...
# prompt (so relevant context is guaranteed). Prefetch can be disabled
# with MEMGO_PREFETCH=false.
#
# Input:  JSON on stdin (prompt, session_id, cwd, transcript_path)
# Output: Additional context injected into the response (exit 0)
...
MSG_COUNT_FILE="/tmp/memgo_msg_count_${USER:-default}"
MSG_COUNT=0
...
# No API key — emit detections only, skip prefetch
if [ -z "${MEMGO_API_KEY:-}" ]; then
  _PROMPT_CTX=""
  if [ -n "$HAS_ERROR" ]; then
    _PROMPT_CTX="Error detected in prompt. Set MEMGO_API_KEY to search past debugging context."
  fi
...
if [ -n "$HAS_RESUME" ]; then
  RESUME_RESULTS=$(PYTHONPATH="$SCRIPT_DIR" MEMGO_SEARCH_USER="$MEMGO_RESOLVED_USER_ID" python3 -c "
import os, sys
...
state = search_memories(user_id, agent_id, 'session state current task', top_k=3)
decisions = search_memories(user_id, agent_id, 'recent decisions and learnings', top_k=3)
...
if unique:
    print(format_results_for_context(unique, heading='Session context recovered from MemGo'))
    print()
    print('These memories provide context for resuming work.')
else:
    print('No session state found in MemGo.')
" 2>/dev/null || echo "")
...
# Query-driven prefetch: search MemGo with the current prompt and inject the top
...
# MEMGO_PREFETCH=false.
if [ -z "$HAS_RESUME" ] && [ "${MEMGO_PREFETCH:-true}" != "false" ]; then
  PREFETCH_RESULTS=$(PYTHONPATH="$SCRIPT_DIR" MEMGO_SEARCH_USER="$MEMGO_RESOLVED_USER_ID" MEMGO_SEARCH_QUERY="$PROMPT" python3 -c "
import os, sys
sys.path.insert(0, os.environ.get('PYTHONPATH', '.'))
from _search import search_memories, format_results_for_context
...
if [ -n "$HAS_REMEMBER" ]; then
  _PROMPT_CTX="${_PROMPT_CTX:+${_PROMPT_CTX}\n}Remember intent detected. Store it verbatim via add_memory with confidence=1.0."
fi

if [ -n "$HAS_ERROR" ]; then
  _PROMPT_CTX="${_PROMPT_CTX:+${_PROMPT_CTX}\n}Error detected in prompt. Prior occurrences may be stored in MemGo — search before answering."
fi
...
# Auto-capture: directly call MemGo OSS API in background every 3rd message.
# At MSG_COUNT=3 the 3rd response isn't in the transcript yet (hook fires
# before the agent responds), so we capture 4 exchanges instead of 3. The
...
if [ "${MEMGO_AUTO_SAVE:-true}" != "false" ] && [ $((MSG_COUNT % 3)) -eq 0 ] && [ "$MSG_COUNT" -gt 0 ] && [ -n "$TRANSCRIPT_PATH" ]; then
...
_ADDS=0
_STATS_FILE="/tmp/memgo_session_stats_${USER:-default}.json"
