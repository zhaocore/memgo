...
按会话跟踪记忆写入/搜索次数。
使用 /tmp/memgo_session_stats_$USER.json(每个用户单文件, init 时重置)。

Usage:
  python3 session_stats.py init            # 新会话重置
  python3 session_stats.py add <category>  # 记录一次记忆写入
  python3 session_stats.py search          # 记录一次搜索
  python3 session_stats.py report          # 打印摘要, 清理临时文件
"""
