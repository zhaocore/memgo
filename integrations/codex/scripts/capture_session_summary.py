...
每次 Stop(每个 assistant turn 结束)触发。每次调用读取 transcript JSONL,
提取最新一条 assistant 消息与所有触及的文件, 通过 MemGo OSS 面同步写入
infer=True 提取摘要。用 run_id=session_id 把摘要限定到会话范围, 使最终
存储的摘要反映最近一个 turn, 而非仅第一次。
...
log = logging.getLogger("memgo-session-summary")
log.setLevel(logging.DEBUG)
_handler = logging.StreamHandler(sys.stderr)
_handler.setFormatter(logging.Formatter("[memgo-session-summary] %(message)s"))
...
if os.environ.get("MEMGO_DEBUG"):
    ...
    try:
        ...
        _fh.setFormatter(logging.Formatter("[memgo-session-summary] %(asctime)s %(message)s"))
        log.addHandler(_fh)
    except OSError:
        pass
...
def extract_last_assistant_message(lines: list[str]) -> str:
    """反向遍历 transcript, 返回最后一条 assistant 消息的文本内容。"""
...
def extract_files_touched(lines: list[str]) -> list[str]:
    """从 transcript 的 tool_use content blocks 提取唯一文件路径。"""
    files = set()
...
def build_summary_prompt(assistant_msg: str, files: list[str]) -> str:
    """构造结构化提示, 帮助 MemGo server 提取好的摘要。"""
    files_section = ""
    if files:
...
def store_summary(
    ...
    # summary_prompt 包裹了 assistant 自己的最后一条消息。MemGo server 从每条
    # 消息提取"关于用户的事实", role 是判断说话者身份的唯一信号, 故这里用
    # role="assistant" 让助手自己的陈述保留归属。
    try:
        results = add_memory(
        ...
        log.info("Session summary stored (%d memory(-ies))", len(results))
        return True
...
def main():
    ...
    store_summary(summary_prompt, user_id, session_id, project_id, branch, files)
