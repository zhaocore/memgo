...
def _print_status(value: dict) -> None:
    ...
    print(f"MemGo: {'paused' if value['paused'] else 'active'}")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "status":
            ...
            print("MemGo stopped saving and searching memories.")
        elif args.command == "resume":
            store.set_setting("paused", "false")
            print("MemGo resumed saving and searching memories.")
        elif args.command == "forget":
            if not args.yes:
                print(
                    "Refusing to delete data without --yes. Add --remote to also "
                    "delete this user/repository scope from MemGo."
                )
                return 2
            remote_result = (
                forget_remote_repo(
