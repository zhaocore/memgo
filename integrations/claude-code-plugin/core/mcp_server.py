#!/usr/bin/env python3
"""把 MemGo 的记忆搜索暴露为本地 Claude Code 的单个工具。

调用 MemGo server 的 OSS /search 接口。
"""
...
TOOL_DESCRIPTION = (
    ...
    "preferences, 'dir' behaves the same as 'repo' because the OSS server has "
    "no directory filtering, and 'mine' is your preferences alone. Pass "
...
TOOL_SCHEMA = {
    ...
    "properties": {
        ...
        "top_k": {
            ...
            "minimum": 1,
            "maximum": 20,
            "description": "Maximum memories to return. Uses MemGo's configured default when omitted.",
        },
        "scope": {
            "type": "string",
            ...
            "description": (
                ...
                "shared memory plus your own preferences, 'dir' behaves the same as "
                "'repo', 'mine' is your preferences alone."
            ),
        },
        "run_id": {
            "type": "string",
...
def _validate_arguments(
    ...
    unknown = set(arguments) - {"query", "top_k", "scope", "run_id"}
    if unknown:
    ...
    return query, top_k, scope, run_id


def call_search_memories(arguments: Any) -> str:
    query, top_k, scope, run_id = _validate_arguments(arguments)
...
def handle_request(message: Any) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        ...
        return {
            ...
            "result": {
                ...
                "serverInfo": {"name": "memgo", "version": "0.3.0"},
            },
        }
    if method == "ping":
    ...
    if method == "tools/call":
        ...
        if params.get("name") != TOOL_NAME:
            result = _tool_response("Unknown MemGo tool.", is_error=True)
        else:
            try:
