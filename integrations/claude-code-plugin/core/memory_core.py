#!/usr/bin/env python3
"""保存有用的编码记忆,并在后续 Claude Code 会话中检索它们。

Hooks 在本地记录会话细节。当 Claude 压缩或结束会话时,MemGo 会把会话的
有用部分同步写入 MemGo server 的 OSS 面,由 server 抽取生成记忆。Claude
可以在之后的工作中检索这些记忆。

API 层指向 MemGo 自托管 server 的 OSS 契约:同步写入、X-API-Key 鉴权、
无事件轮询、无 category。
"""
...
DEFAULT_API_URL = "https://memgo.wxget.com"
...
# NOTE: 托管平台通过 agent_custom_instructions/custom_instructions/custom_categories
# 让平台按"项目记忆/个人记忆"两类指令抽取。OSS 面没有这些字段,抽取指令由 server
# 统一管理,因此这些平台机制与对应字段一并删除。
...
SECRET_PATTERNS = [
    ...
    re.compile(r"\b(?:sk|m0|psk)-[A-Za-z0-9_\-]{12,}\b"),
    # MemGo server 的 API key 前缀是 mgsk_。
    re.compile(r"\bmgsk_[A-Za-z0-9_\-]{12,}\b"),
...
def _search_filters(user: str, repo: RepoContext, scope: str) -> dict[str, Any]:
    """Build the scope filter for the OSS /search endpoint.

    OSS 面只有 user_id/agent_id/run_id 三个过滤维度,没有 app_id 与 category。
    scope 映射: repo=仅 agent_id(项目共享记忆,含该仓库下所有记忆);
    dir=与 repo 相同(OSS 无目录过滤,目录维度已在 metadata.dirs 记录,注释说明为简化);
    mine=user_id+agent_id(个人在仓库内的记忆)。
    """
    if scope == "mine":
...
def search_scope() -> str:
    configured = (
        _plugin_option("search_scope", "MEMGO_CODE_SEARCH_SCOPE") or ""
    ).strip().lower()
...
def _legacy_project_map(cwd: str, root: str, raw_remote: str) -> str:
    """Return the project name used by the previous MemGo plugin (位于 ~/.memgo)。"""
    try:
        data = json.loads((Path.home() / ".memgo" / "project_map.json").read_text())
...
def _legacy_project_id(cwd: str, root: str, raw_remote: str, identity: str) -> str:
    """Use the repository namespace created by the previous MemGo plugin."""
    configured = _scope_value(os.environ.get("MEMGO_PROJECT_ID"))
    if configured:
        return configured
...
def api_key() -> str:
    configured = (
        os.environ.get("MEMGO_API_KEY")
        # Claude hook-only 配置以 plugin option 形式注入(plugin.json 里 MEMGO_API_KEY)。
        or os.environ.get("CLAUDE_PLUGIN_OPTION_MEMGO_API_KEY")
        or ""
    ).strip()
    if configured:
        return configured
...
def cache_plugin_api_key() -> bool:
    ...
    configured = os.environ.get("CLAUDE_PLUGIN_OPTION_MEMGO_API_KEY") or ""
    if not configured:
        return False
...
def user_id() -> str:
    return (
        _scope_value(_plugin_option("user_id", "MEMGO_CODE_USER_ID"))
        or _scope_value(os.environ.get("MEMGO_USER_ID"))
...
def data_dir() -> Path:
    configured = os.environ.get("MEMGO_CODE_DATA_DIR") or os.environ.get(
        "CLAUDE_PLUGIN_DATA"
    )
    return (
        Path(configured).expanduser()
        if configured
        else Path.home() / ".memgo" / "claude-code-plugin"
    )
...
class EvidenceStore:
        ...
        existing = self.conn.execute(
            """SELECT * FROM flushes
               WHERE repo_id = ? AND session_id = ?
                 AND status NOT IN ('success', 'gave-up', 'nothing-to-flush')
    ...
    def update_flush(self, packet_id: str, **fields: Any) -> None:
        ...
        failed = str(fields.get("status", "")) == "error"
        if failed:
...
def record_sidekick_start(
    ...
    agent_type = bounded(hook_input.get("agent_type", "memgo:sidekick"), 200)
...
def build_extraction_messages(structured: dict[str, Any]) -> list[dict[str, str]]:
    """Build the session messages sent to MemGo for memory extraction."""
...
def touch_handoff_heartbeat() -> None:
    ...
    path = os.environ.get("MEMGO_CODE_HANDOFF_PATH", "")
    if not path:
        return
    try:
        os.utime(path)
...
def _record_flush(
    ...
    **extra: Any,
    ...
    telemetry.record(
        ...
        reason=reason,
        status=status,
        success=status in {"success", "nothing-to-flush"},
...
def flush_session(
    ...
    metadata = {"source": "memgo_claude_plugin"}
    if repo.branch and repo.branch not in {"detached", "unknown"}:
        metadata["branch"] = repo.branch
    if repo.head_sha:
        metadata["git_sha"] = repo.head_sha
    api_url = os.environ.get("MEMGO_BASE_URL", DEFAULT_API_URL).rstrip("/")
    add_url = f"{api_url}/memories"
    ...
    # OSS 写入同步返回 results,无需平台事件轮询。
    # 范围: user_id=个人、agent_id=项目、run_id=会话;写入即三条 lane 都有归属。
    body = {
        "user_id": write_user,
    ...
    try:
        ...
        if not batches:
            store.update_flush(packet_id, status="nothing-to-flush", error="")
        ...
        for _, messages in batches:
            response, req_size, resp_size = _request_json(
                add_url, key, {**body, "messages": messages}, 60
            )
            results = response.get("results") if isinstance(response, dict) else []
            memory_count += len(results) if isinstance(results, list) else 0
            request_chars += req_size
            response_chars += resp_size

        elapsed = (time.perf_counter() - started) * 1000
        store.update_flush(packet_id, status="success", error="")
        store.operation(
            repo,
            session_id,
            "flush",
...
def search_memories(
    ...
    *,
    ...
    search_once = os.environ.get(
        "MEMGO_CODE_SEARCH_ONCE_PER_SESSION", "false"
    ).lower() in {
        "1",
        "true",
        "yes",
    ...
    result_limit = min(
        max(
            ...
            else _int_option("top_k", "MEMGO_CODE_TOP_K", 3),
            1,
        ),
        20,
    )
    ...
    url = (
        os.environ.get("MEMGO_BASE_URL", DEFAULT_API_URL).rstrip("/") + "/search"
    )
    started = time.perf_counter()
    try:
...
def _scoped_memory_ids(
    ...
    OSS 面没有 app_id:仓库范围由 agent_id(=project_id)表达,不再按 app_id 前缀过滤。
    """
    ids: list[str] = []
    seen: set[str] = set()
    _collect_memory_ids(
        api_url, key, {"user_id": user, "agent_id": repo.project_id}, ids, seen
    )
    if include_project:
        # 项目共享记忆:仅 agent_id 一条 lane。
...
def _collect_memory_ids(
    ...
    """通过 GET /memories 列表接口收集记忆 id。

    OSS 列表无分页游标,一次性取满(server 的 top_k 上限 1000)。
    平台 /v2/memories/?page= 分页机制已删除。
    """
    query = urllib.parse.urlencode({**filters, "top_k": FORGET_PAGE_SIZE})
    parsed, _ = _get_json(f"{api_url}/memories?{query}", key, 15)
...
def _delete_memory(api_url: str, key: str, memory_id: str) -> bool:
    request = urllib.request.Request(
        f"{api_url}/memories/{urllib.parse.quote(memory_id)}",
        headers={"X-API-Key": key, "Content-Type": "application/json"},
        method="DELETE",
    )
    try:
...
def forget_remote_repo(
    ...
    if not key:
        ...
        return {"status": "error", "error": "MemGo API key is not configured"}
...
def _doctor_memgo_authentication(repo: RepoContext) -> dict[str, Any]:
    """Verify the configured key with one read-only, repository-scoped search."""
    key = api_key()
    if not key:
        return {"ok": False, "detail": "API key missing"}
    payload = {
        "query": "MemGo authentication check",
        "filters": {"user_id": user_id(), "agent_id": repo.project_id},
        "top_k": 1,
        "threshold": 1.0,
    }
    url = os.environ.get("MEMGO_BASE_URL", DEFAULT_API_URL).rstrip("/")
    started = time.perf_counter()
    try:
        _request_json(f"{url}/search", key, payload, 5)
    except Exception as exc:
...
def _doctor_user_id() -> dict[str, Any]:
    ...
    configured = _plugin_option("user_id", "MEMGO_CODE_USER_ID") or os.environ.get(
        "MEMGO_USER_ID", ""
    )
...
def doctor(cwd: str | None = None) -> dict[str, Any]:
    ...
    checks: dict[str, dict[str, Any]] = {
        ...
        "memgo_authentication": _doctor_memgo_authentication(repo),
    }
    return {
