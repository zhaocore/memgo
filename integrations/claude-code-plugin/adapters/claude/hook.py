...
def first_prompt_memory_output(store: EvidenceStore, hook_input: dict) -> dict:
    ...
    if not is_first_prompt:
        return {}
    try:
        minimum_query_chars = int(os.environ.get("MEMGO_CODE_MIN_QUERY_CHARS", "20"))
    except ValueError:
    ...
    context = format_context(
        result.memories,
        "MemGo found these relevant memories from earlier work in this repository:",
    )
    telemetry.record(
        "context_injected",
        repo=repo,
...
def automatic_flush_enabled() -> bool:
    return os.environ.get("MEMGO_CODE_AUTO_FLUSH", "true").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _idle_flush_seconds() -> int:
    try:
        return max(
            int(os.environ.get("MEMGO_CODE_IDLE_FLUSH_SECONDS", str(DEFAULT_IDLE_FLUSH_SECONDS))),
            0,
        )
    except ValueError:
...
def main() -> int:
    ...
    if args.plugin_data_dir:
        os.environ["MEMGO_CODE_DATA_DIR"] = args.plugin_data_dir
    cache_plugin_api_key()
    ...
    try:
        ...
        if args.action == "session-start":
            ...
            if os.environ.get("MEMGO_CODE_SYNC_FLUSH") == "1":
