...
| `server/` | HTTP 层（doc-02 合同）：store+goose 迁移、auth 三层依赖、middleware、api、errpkg；`server/webhook` 为记忆事件投递器（`/webhooks` 注册表）；`/analytics` 聚合 `request_logs`（admin）、`/export` 记忆导出（json/csv/schema，全量导出限 admin）。 |
