"""memgo whoami — print the active agent's default_user_id (AGENTRUSH identifier)."""
...
from memgo_cli.branding import BRAND_COLOR, print_error, print_info
...
def run_whoami() -> None:
    ...
    if not session_id:
        print_error(
            err_console,
            "No default_user_id found. Run `memgo init --agent` first.",
        )
        raise typer.Exit(1)
