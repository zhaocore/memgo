...
from memgo_cli.backend.json import json_records
...
from memgo_cli.output.format import format_agent_envelope, format_json
...
def cmd_event_list(backend: Backend, *, output: str) -> None:
    """显示最近的后台处理事件。"""
...
def cmd_event_status(backend: Backend, event_id: str, *, output: str) -> None:
    """显示指定后台事件的状态。"""
    from memgo_cli.runtime.state import is_agent_mode, set_current_command
    ...
    results = json_records(ev.get("results") or [])
